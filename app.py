...
with status_tab:
    ...
    st.markdown(
        ...
        "- 매출·매입·재고 모두 창고명이 `총판`인 행만 분석에 포함\n"
        "- 반품 수량과 금액은 음수, 출고·입고는 양수로 표준화\n"
        "- 현재 재고는 계산하지 않고 ERP 재고 파일의 수량을 직접 사용\n"
        "- 아레나 9자리 품번은 생산연도 자리를 제거해 통합상품군으로 자동 분류하며, 영문 생산연도는 `기타연도`로 표시\n"
...
with mapping_tab:
    ...
    st.markdown(
        "`브랜드 1자리 + 생산연도 1자리(숫자 또는 영문) + 대분류 2자리 + 생산구분 1자리 + 핵심코드 4자리` "
        "규칙을 적용합니다. 예: `A1AC1AC01`, `A3AC1AC01`, `AYAC1AC01` → `A|AC|1|AC01`. "
        "영문 생산연도는 모두 `기타연도`로 표시합니다."
    )
    if inventory is None:
